...
public class SpaceBar : MonoBehaviour
{
    ...
    private float nextActionTime = 0.0f;        //used to compare with local time, incremented by period amount when needed.
    public float period = 0.1f; //time before a sign is recognized/printed

    private float nextResetTime = 0.0f;
    public float resetPeriod = 0.1f; //time before the text resets

    /*text to insert to the default text given to user for a sign to try*/
    string signToTry = "Open Palm";
    ...
    void handleFrameReady(object sender, EventArgs e)
    {
        ...
        { 
            ...
            int i = 0;
            Hand compareHand;
            if (framey.CurrentFramesPerSecond > 0 && framey.Hands.Count > 0)        //if there are multiple tracking frames and hand in frame
            {
